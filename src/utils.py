...
# This function saves the trained pipeline/model to a local file in the models directory using joblib. It returns the path to the saved file.
...
# This function generates a SHA-256 hash of the input payload dictionary. It first converts the dictionary to a JSON string with sorted keys to ensure consistent hashing, and then computes the hash of the string.
def hash_input(payload: dict) -> str:
    payload_str = json.dumps(payload, sort_keys=True, default=str)
    return hashlib.sha256(payload_str.encode()).hexdigest()


# This function writes a prediction log entry to a log file in the logs directory. Each log entry includes the timestamp, model name, model version, input hash, and the prediction result. The log is stored in JSON format, with one entry per line.
